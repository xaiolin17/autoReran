...
class GoodsCreate(BaseModel):
    sku: Optional[str] = Field(
        None, description="产品SKU编码  *必填", examples=["sku_1001"]
    )
    name: str = Field(..., description="商品名称  *必填", examples=["百岁山1L"])
    price: Decimal = Field(
        ..., description="商品售价，单位：元  *必填", examples=[10.0]
    )
    ...
    cost_price: Optional[Decimal] = Field(
        None, description="商品成本价，单位：元", examples=[6.0]
    )
    promotion_price: Optional[Decimal] = Field(
        None, description="商品促销售价，单位：元", examples=[8.0]
    )
    virtual_sales: Optional[int] = Field(
        None, description="虚拟销量，用于展示", examples=[100]
    )
    ...
    description: Optional[str] = Field(
        None, description="商品描述", examples=["优质天然矿泉水"]
    )
    use_type: Optional[int] = Field(None, description="商品用途类型 ID", examples=[1])
    use_type_name: Optional[str] = Field(
        None, description="商品用途类型名称", examples=["自用"]
    )
    alias: Optional[str] = Field(None, description="商品别名", examples=["大瓶水"])
    self_pickup_price: Optional[Decimal] = Field(
        None, description="商品自提价，单位：元", examples=[9.0]
    )
    stock: Optional[int] = Field(
        None, description="库存数量，必须为非负整数", examples=[50]
    )
    warning_value: Optional[int] = Field(10, description="库存预警值", examples=[10])
    upstairs_delivery_fee: Optional[Decimal] = Field(
        None, description="上楼配送费(步梯)"
    )
