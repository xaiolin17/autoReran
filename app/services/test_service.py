...
#             category = select(Category.name).where(
#                 Category.id == goods_obj.type
#             )
...
#             unit = select(SysDictData.value).where(
#                 SysDictData.id == goods_obj.unit
#             )
...
#             if user_id:
#                 user_expand_info = await self.db.get(
#                     UserExpandModel, user_id
#                 )
...
#                 delete(Goods).where(Goods.id == db_goods_info.id)
#             )
#             await self.db.commit()
#
#             if (
...
#                 and e.args[0].get("detail")
#             ):
#                 raise Exception(
#                     {"detail": e.args[0].get("detail"), "exception": e}
#                 )
#             raise Exception(
#                 {"detail": "新增商品时发生错误", "exception": e}
#             )
