...
class BacktestService:
    ...
    def run_backtest(self, request: BacktestRequest) -> BacktestResult:
        ...
        db_backtest.start_date = (
            df.iloc[0]["datetime"].isoformat()
            if hasattr(df.iloc[0]["datetime"], "isoformat")
            else str(df.iloc[0]["datetime"])
        )
        db_backtest.end_date = (
            df.iloc[-1]["datetime"].isoformat()
            if hasattr(df.iloc[-1]["datetime"], "isoformat")
            else str(df.iloc[-1]["datetime"])
        )
