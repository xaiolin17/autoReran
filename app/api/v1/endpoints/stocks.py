...
def update_mark(
    ...
    返回值说明:
        dict: {"message": str, "stock_code": str, "date": str, "label": str}，表示更新结果
...
def fetch_and_save_stock_data(
    ...
    返回值说明:
        dict: {"message": str, "count": int, "start_date": str, "end_date": str}，表示保存结果
...
def fetch_async(
    ...
    返回值说明:
        dict: {"message": str, "stock_code": str, "start_date": str, "end_date": str}，表示下载已启动
    ...
    关键逻辑说明:
        ...
        2. 通过 WebSocket 向客户端推送下载进度（downloading -> calculating -> completed/error）
        3. 下载完成后自动计算技术指标
    """
...
def refresh_stock_data(
    ...
    返回值说明:
        dict: {"message": str, "stock_code": str, "period": str, "incremental": bool}，表示刷新状态
...
def deduplicate_stock_data(
    ...
    返回值说明:
        dict: {"message": str, "stock_code": str, "period": str, "deleted_count": int}，表示清理结果
...
def _send_progress_ws(stock_code: str, status: str, progress: int, message: str, new_data_available: bool = False):
    ...
    参数说明:
        ...
        status (str): 下载状态，可选值：downloading（下载中）、calculating（计算中）、completed（完成）、error（错误）
...
def _run_fetch_task(stock_code: str, period: str, start_date: Optional[str] = None, end_date: Optional[str] = None):
    ...
    关键逻辑说明:
        ...
        4. 如果有新数据，发送 calculating 进度通知（70%），并调用 IndicatorService.calculate_and_save_indicators() 计算技术指标
...
def _run_force_refresh_task(stock_code: str, period: str):
    ...
    关键逻辑说明:
        ...
        4. 调用 StockService.fetch_and_save_stock_data(incremental=False, force=True) 强制下载完整数据并对比更新
